"""
08_resume_train.py - Resume MovementGNN Training
=================================================
Loads checkpoint_best.pt from 07_train_gnn.py and continues training
with additional epochs and a warm restart of the cosine scheduler.
"""
...
_v1 = get_training_paths("v1")
_v2 = get_training_paths("v2")
GRAPH_PATH     = _v1["graph"]
EMB_DIR        = _v1["emb_dir"]
RESUME_CKPT    = _v1["out_dir"] / "checkpoints/checkpoint_best.pt"
OUT_DIR        = _v2["out_dir"]
USE_EMBEDDINGS = True
...
class DAMEPLANLoss(nn.Module):
    ...
    def __init__(self, weight_ce=1.0, weight_phys=0.5, class_weights=None):
        super().__init__()
    ...
    def forward(self, outputs, targets, features):
        ...
        for i, frac in enumerate(FRACTIONS):
            ...
            if not valid.any():
                continue
            loss_t = self.weight_ce * self.focal(logits[valid], labels[valid])
...
def export_geotiff(preds_dict, raster_meta, pos, out_dir, tag="gnn"):
    ...
    with rasterio.open(mpath, "w", driver="GTiff", height=H, width=W,
        ...
        dst.update_tags(MODEL=tag, CLASSES="1=Go,2=SlowGo,3=NoGo")
...
def main():
    ...
    print("=" * 70)
    print("  MOVEMENT GNN — RESUME TRAINING (Pacaraima)")
    print("=" * 70)
    log(f"Device: {device} | AMP: {'bfloat16' if use_amp else 'OFF'}")
    log(f"Resume from: {RESUME_CKPT.name}")
    if device == "cuda":
    ...
    if is_hetero:
        ...
        log(f"  DEM: {n_nodes:,} nodes, {in_ch} features")
        log(f"  Edges dem-dem: {n_edges_dem:,}")
        for nt in data.node_types:
            if nt != "dem":
                log(f"  {nt}: {data[nt].x.size(0) if hasattr(data[nt], 'x') else '?'} nodes")
        ...
        log(f"  Nodes: {n_nodes:,}, features: {in_ch}")
        input_nodes = None
    ...
    if emb_path and emb_path.exists():
        ...
        log(f"  No embeddings - using {in_ch} base features")
    ...
    # --- 6. Model + Optimizer + Scheduler + Resume ---
    has_edge_attr = False
    if is_hetero:
    ...
    loss_fn = DAMEPLANLoss(weight_ce=1.0, weight_phys=0.5).to(device)
    ...
    log(f"Loading checkpoint: {RESUME_CKPT.name} ...")
    ...
    best_val_loss = bm.get("val_loss", float("inf"))
    best_val_acc = bm.get("val_acc", 0.0)

    prev_config = ckpt.get("config", {})
    prev_history = prev_config.get("training_history", [])

    log(f"  Resumed from epoch {start_epoch} | val_loss={best_val_loss:.5f} | val_acc={best_val_acc:.3f}")
    log(f"  Previous F1: {bm.get('f1', {})}")
    log(f"  New LR: {LR} | Extra epochs: {EXTRA_EPOCHS} | Total: {start_epoch + EXTRA_EPOCHS}")
    log(f"  MovementGNN: {n_params:,} params | in={in_ch} hidden={HIDDEN_DIM} "
        f"layers={NUM_LAYERS} heads={HEADS} edge_dim={edge_dim}")
    log(f"  Scheduler: CosineAnnealingLR (T_max={EXTRA_EPOCHS}, warm restart)")
    log(f"  AMP: {'bf16' if use_amp else 'OFF'} | Grad clip: {MAX_NORM}")

    del ckpt
    gc.collect()

    # --- 7. Training Loop (Resume) ---
    OUT_DIR.mkdir(parents=True, exist_ok=True)
    ...
    config_dict = {
        ...
        "epochs_extra": EXTRA_EPOCHS, "epochs_prev": start_epoch,
        "epochs_total": start_epoch + EXTRA_EPOCHS,
    ...
    log(f"Starting training: epochs {start_epoch+1}..{total_epochs} "
        f"({EXTRA_EPOCHS} epochs x {n_train_batches} batches)")
    print("=" * 70, flush=True)
    ...
    for epoch in range(start_epoch + 1, total_epochs + 1):
        ...
        log(f"Epoch {epoch:02d}/{total_epochs} | "
    ...
    report = [
        "=" * 70,
        "FINAL REPORT - MovementGNN RESUME",
        "=" * 70,
        f"Total time: {total_time:.0f}s ({total_time / 60:.1f} min)",
        f"Previous epochs: {start_epoch} | Additional: {EXTRA_EPOCHS}",
