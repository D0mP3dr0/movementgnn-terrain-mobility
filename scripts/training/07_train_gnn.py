"""
07_train_gnn.py - MovementGNN Training (V1)
============================================
Standalone training script for Google Colab.

Strategy (aligned with GNN_RF train_lins_physics.py):
  - NeighborLoader: mini-batches of DEM nodes
  - Each batch samples K=8 neighbors per node (DEM-DEM)
  - GPU VRAM: ~2-4 GB per batch with AMP float16
  - Full HeteroData kept in RAM (no conversion to homogeneous Data)

Graph: HeteroData
  - dem: 12M nodes x 17 features
  - sentinel: 108M nodes
  - lidar: 400k nodes
  - edges: dem-dem, sentinel-dem, lidar-dem, lidar-lidar

Embeddings (optional):
  - GNN_TOPO embeddings concatenated to dem.x
...
Loss: DAMEPLANLoss (CrossEntropy + FocalLoss + physics constraints)

Output: checkpoints, metrics.json, predictions.npz, GeoTIFF per force type
"""
...
# Feature indices in dem.x tensor (17 base features)
IDX_SLOPE = 1
IDX_NDVI  = 12
IDX_NDWI  = 13
IDX_WATER = 14
...
def main():
    ...
    print("=" * 70)
    print("  MOVEMENT GNN — MILITARY MOVEMENT RESTRICTION (Pacaraima)")
    print("=" * 70)
    ...
    # --- 2. Concatenate GNN_TOPO Embeddings (optional) ---
    emb_path = None
    if USE_EMBEDDINGS:
    ...
    log(f"Starting training: {EPOCHS} epochs x {n_train_batches} batches")
    print("=" * 70, flush=True)
    ...
    export_geotiff(preds_np, raster_meta, pos, OUT_DIR, tag="movement_gnn")

    total_time = time.perf_counter() - start_time
    report = [
        "=" * 70,
        "FINAL REPORT - MovementGNN",
        "=" * 70,
